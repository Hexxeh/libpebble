...
class _Analytics(object):
    ...
    def __init__(self):
        ...
        if _running_in_vm():
            self.os_str += " (VM)"
        self.user_agent = 'Pebble SDK/%s (%s-python-%s)' % (
            cur_sdk_version,
            self.os_str,
        ...
        try:
            cached_version = open(
                os.path.join(settingsDir, "sdk_version")
            ).read()
        except:
            cached_version = None
        ...
        for key, value in data.items():
            if isinstance(value, basestring):
                if isinstance(value, unicode):
                    ...
                    data[key] = unicode(value, errors='replace').encode(
                        'utf-8')
...
def missing_tools_evt():
    ...
    _Analytics.get().post_event(category='install', action='tools',
                                label='fail: The compiler/linker tools could'
                                      ' not be found')
...
def res_sizes_evt(uuid, resCounts, resSizes):
    ...
    for key in resSizes.keys():
        _Analytics.get().post_event(category='appResources',
                                    action='%sSize' % (key), label=uuid,
                                    value=resSizes[key])
        _Analytics.get().post_event(category='appResources',
                                    action='%sCount' % (key), label=uuid,
                                    value=resCounts[key])
